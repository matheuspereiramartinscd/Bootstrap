...
addList=()=>{
   ...
   if (inputText) {
    list.innerHTML += ` <li class=" my-3 py-3 shadow list-group-item " id="list${listNum}">
                ...
                <div class="col-4">
                     <button class=" btn btn-dark" onclick="deleteList(${listNum})">Deletar</button>
...
filterList=(x)=>{
       if (x) {
            if (x.length >= minimalValue) {
                ...
                alert("Por favor digite mais que 3 letras")
            }
       }
       else{
            return false;
...
editList=(listId)=>{
    ...
    let newText = prompt("Quer trocar a lista?",currentText.innerHTML);
...
deleteList=(listId)=>{
    let current = document.getElementById(`text${listId}`).innerHTML;
       let deleteComfirm = confirm(`Tem certeza em deletar a tarefa${current}?`);
    if (deleteComfirm) {
